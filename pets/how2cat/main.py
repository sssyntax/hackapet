...
game_active = False  # Assuming the game starts active
...
def update_love_bar():
    ...
    elif love_points >= 75:  # Added condition for 75 points
...
#game
def play():
    global love_points, frame, game_active
    game_active = True  # Indicate that the game has started
    score = 0
    hearts = 3
    game_paused = False
    game_over = False
    ...
    try:
        ...
        game_group.append(heart_sprite)
        ...
        if font:
            ...
            game_group.append(game_score_label)
    ...
    game_group.append(tabucat_play_sprite)
    ...
    # Game loop
    while hearts > 0 and not game_over:
    ...
    try:
        game_group.remove(game_score_label)
        print("Game score label removed from game group.")
    except ValueError:
        pass

    game_active = False  # Indicate that the game has ended
...
def show_game_over_screen():
    ...
    game_over_label = label.Label(
        ...
        x=(display.width - 100) // 2,  # Center horizontally
        y=(display.height - 50) // 2  # Center vertically
    )
    game_over_group.append(game_over_label)

    # Create a label for restart instructions
    restart_label = label.Label(
        font,
        text="Press R to Restart or Q to Quit",
        color=0xFFFFFF,
        x=(display.width - 250) // 2,
        y=(display.height - 20) // 2 + 40  # Below the game over message
    )
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_r:  # Restart the game
                    splash.remove(game_over_group)  # Remove the game over screen
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_ESCAPE:  # Quit with ESC
                running = False
            elif event.key == pygame.K_e and not brushing_active:
                start_brushing()
            elif event.key == pygame.K_g:  # Start the game when G is pressed
                play()
            elif event.key == pygame.K_o and not petting_active:  # Start petting when O is pressed
                start_petting()  # Call the function to start petting
            elif event.key == pygame.K_f and not feeding_active:
                start_feeding()
            elif event.key == pygame.K_r:  # Restart the game when R is pressed
                if show_game_over_screen():  # Check if the game should restart
                    love_points = 10  # Reset love points to 10
                    game_active = False  # Ensure game_active is reset
    ...
    if running:  # Only update if running is still True
        ...
        # love_points = max(0, love_points - 0.5)  # Uncomment for auto-decay
        update_love_bar()
        ...
        if not (brushing_active or petting_active or game_active or feeding_active):  # Only decay when not active
            love_points = max(0, love_points - 0.1)  # Slowly decrease love points
            update_love_bar()  # Update the love bar display
